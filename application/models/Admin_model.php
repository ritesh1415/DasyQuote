...
class Admin_model extends CI_Model
{
	...
	{
		$this->db->select('bs.id,bs.currency_code,s.service_title,p.name as providername,u.name as username,cod.amount,cod.status');
   ...
   public function getSingleData($table,$where=array()) {
    ...
    {
        $query  = $this->db->query("SELECT * FROM  `country_table` ORDER BY id DESC");
        $result = $query->result_array();
        return $result;
    }
...
//changes made by gouresh 
	public function contactreply_list($id)
{
    $query  = $this->db->query("SELECT cr.*,c.email,c.message FROM `contact_reply` as cr left join contact_form_details as c on cr.contact_id = c.id where cr.contact_id = ?", array($id));
    $result = $query->result_array();
    return $result;
}
//changes made by gouresh
