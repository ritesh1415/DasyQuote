...
class Api extends REST_Controller
{
    ...
    public function forget_password_post()
    {
        ...
        if (!empty($result)) {
           ...
           /*email settings commented by gouresh     
    ...
    public function subscription_get()
    {
        ...
        if ($this->user_id != 0 || ($this->default_token == $this->api_token)) {
            ...
            $my_subscription = $this->api->get_my_subscription($this->user_id);
            ...
            $datas = $this->db->join('subscription_fee sf', 'sf.id = sd.subscription_id', 'left')->get_where('subscription_details sd', array('sd.subscriber_id' => $this->user_id, 'DATE(sd.expiry_date_time)>=' => date('Y-m-d'), 'sd.type' => 1, 'sf.status' => 1))->result();
            ...
            if ($result) {
                ...
                if (!empty($user_post_data['from_time']) && !empty($user_post_data['to_time']) && !empty($user_post_data['service_date']) && !empty($user_post_data['service_id']) && !empty($user_post_data['latitude']) && !empty($user_post_data['longitude']) && !empty('location') && !empty($user_post_data['notes']) && !empty($user_post_data['amount'])) {
                    ...
                    if (!empty($from_time)) {
                        ...
                        $response_code = '500';
                        $response_message = $this->language_content['service']['lg_booking_not_available'] . 'sdsdssdsds';
                        $data = new stdClass();
                        $res = $this->data_format($response_code, $response_message, $data);
                ...
                if ($result) {
                    ...
                    if ($booking_status['status'] == '1' && $user_data['status'] == '1') {
                        ...
                        if ($result) {
                            ...
                            $this->send_push_notification($token, $user_data['id'], 2, ' ' . $lang_value);
                        ...
                        if (!empty($user_data['id']) && !empty($user_data['status']) && !empty($user_data['type'])) {
                            ...
                            if ($booking_status['status'] == '3' && $user_data['status'] == '4') {
                                ...
                                if ($result) {
                                    ...
                                    $lang_value = $this->getNotificationLangText($user_language, $page_key, $lang_key);
                                    /* completed user site */
                                    $this->send_push_notification($token, $user_data['id'], 1, ' ' . $lang_value);
    ...
    public function review_type_get()
    {
        ...
        if (!empty($rating_type)) {
            $response_code = '200';
            $response_message = $this->language_content['signin']['lg_review_type_list'];
