...
<div class="page-wrapper">
	<div class="content container-fluid">
		...
		<div class="page-header">
			<div class="row">
				<div class="col">
					<h3 class="page-title">Blocked Professional</h3>
				</div>
			</div>
		</div>
		<!-- /Page Header -->
                                ...
                                <tbody>
									...
									if(!empty($list)) {
										...
										foreach ($list as $rows) { 
											if($rows['status']==2) {
												$status = "Requested to block provider";
												$color = "info";
											}
											if($rows['status']==1) {
												$status = "Blocked";
											...
											$action = ""; 
											if($rows['status']!=1) {
												$action = "<a href='javascript:;' class='btn btn-sm bg-danger-light mr-2 blockbyAdmin' data-rowid='".$rows['id']."' data-blockedid='".$rows['blocked_id']."' data-blockedbyid='".$rows['blocked_by_id']."' data-usertype='1'><i class='fas fa-ban mr-1'></i> Block </a>";
											} else {
												$action = "<a href='javascript:;' class='btn btn-sm bg-success-light mr-2 unblockbyAdmin' data-rowid='".$rows['id']."' data-blockedid='".$rows['blocked_id']."' data-blockedbyid='".$rows['blocked_by_id']."' data-usertype='1'><i class='fas fa-ban mr-1'></i> UnBlock </a>";
											}
