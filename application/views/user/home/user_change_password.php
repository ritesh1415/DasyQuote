...
function updatepassword()
{
	...
	{
		$("#errchk").html('Confirm password is required').css("color","red");return false;
	}
	var check_pwd = /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[$@!%?&]).{8,30}$/;
	if(confirm_password.match(check_pwd)){
	...
	$.ajax({
		...
		{
			$("#errchk").html('New and Confirm password Mismatched').css("color","red");return false;
			
		}
	} else {
		$("#errchk").html('The password must be more than 8 and less than 30 characters long. The password should contain at least 1 Uppercase Alphabet, 1 Lowercase Alphabet, 1 Number and 1 Special Character.').css("color", "red");
        return false;
	}
}
...
function compcurpwd()
{
	...
	{
		$("#form_submit").prop("disabled",true);
		$("#errchk").html('Current and new password cannot be same ').css("color","red");return false;
	}
	else
	{
		$("#form_submit").prop("disabled",false);
		$("#errchk").html('');return true;
	}
}
</script>